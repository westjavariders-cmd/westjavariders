-- Fix audience menus on the LIVE schema:
-- * Epic Trips lives at slug `epictrips` (not epic-trips), so the previous seed missed it.
-- * Home doors are a separate menu from website_nav_items and were never updated.
-- Idempotent. Does not touch Explore West Java, Meet the Boardriders or Book individually.
...
BEGIN
  ...
  -- Header / drawer: retarget the two old trip items; keep the other three.
  ...
  WHERE n.destination_kind = 'build_your_trip'
     OR lower(btrim(n.internal_name)) = 'build your trip'
     OR EXISTS (
       SELECT 1 FROM public.website_nav_item_translations t
       WHERE t.nav_item_id = n.id
         AND lower(btrim(t.label)) = 'build your trip'
     );
  ...
  WHERE n.destination_page_id IN (
          ...
          WHERE p.slug IN ('epictrips', 'epic-trips', 'epic-trip')
        )
     OR lower(regexp_replace(btrim(n.internal_name), '\s+', ' ', 'g')) IN ('epic trips', 'epic trip')
     OR EXISTS (
       SELECT 1 FROM public.website_nav_item_translations t
       WHERE t.nav_item_id = n.id
         AND lower(regexp_replace(btrim(t.label), '\s+', ' ', 'g')) IN ('epic trips', 'epic trip')
     );

  UPDATE public.website_nav_item_translations t
  SET label = 'Intermediate & Pro'
  WHERE lower(regexp_replace(btrim(t.label), '\s+', ' ', 'g')) IN ('epic trips', 'epic trip');
  ...
  IF NOT EXISTS (
    ...
    VALUES (
      ...
      family_id,
      true,
      COALESCE((SELECT min(sort_order) FROM public.website_nav_items), 0) - 1
    )
  ...
  -- Home mosaic doors (the large tiles on /home). Same three trip names; do not
  -- rename Explore / Meet the Boardriders / Book individually.
  ...
  IF home_id IS NOT NULL THEN
    ...
    IF home_section IS NOT NULL THEN
      FOR door_id IN
        SELECT b.id
        ...
        WHERE b.section_id = home_section
          ...
          AND lower(btrim(t.title)) = 'build your trip'
      LOOP
        ...
        WHERE block_id = door_id AND lower(btrim(title)) = 'build your trip';
      END LOOP;
      ...
      FOR door_id IN
        ...
        WHERE b.section_id = home_section
          AND b.block_kind = 'door'
          AND lower(regexp_replace(btrim(t.title), '\s+', ' ', 'g')) IN ('epic trips', 'epic trip')
      LOOP
        UPDATE public.website_blocks
