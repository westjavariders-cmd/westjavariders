CREATE OR REPLACE FUNCTION public.rename_pricing_variable(_product_id uuid, _old text, _new text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'Only an ADMIN may change product configuration';
  END IF;
  IF _old IS NULL OR _new IS NULL OR _old = _new THEN
    RETURN;
  END IF;

  SELECT id INTO v_pricing_id FROM product_pricing WHERE product_id = _product_id;
  IF v_pricing_id IS NULL THEN
    RETURN;
  END IF;

  UPDATE product_pricing SET
    people_variable   = CASE WHEN people_variable   = _old THEN _new ELSE people_variable   END,
    days_variable     = CASE WHEN days_variable     = _old THEN _new ELSE days_variable     END,
    nights_variable   = CASE WHEN nights_variable   = _old THEN _new ELSE nights_variable   END,
    sessions_variable = CASE WHEN sessions_variable = _old THEN _new ELSE sessions_variable END
  WHERE id = v_pricing_id;

  UPDATE pricing_rules SET
    variable_name      = CASE WHEN variable_name      = _old THEN _new ELSE variable_name      END,
    quantity_variable  = CASE WHEN quantity_variable  = _old THEN _new ELSE quantity_variable  END,
    condition_variable = CASE WHEN condition_variable = _old THEN _new ELSE condition_variable END
  WHERE pricing_id = v_pricing_id;
END;
$function$;

REVOKE ALL ON FUNCTION public.rename_pricing_variable(uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rename_pricing_variable(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rename_pricing_variable(uuid, text, text) TO service_role;

CREATE OR REPLACE FUNCTION public.clear_pricing_variable(_product_id uuid, _name text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  UPDATE product_pricing SET
    people_variable   = CASE WHEN people_variable   = _name THEN NULL ELSE people_variable   END,
    days_variable     = CASE WHEN days_variable     = _name THEN NULL ELSE days_variable     END,
    nights_variable   = CASE WHEN nights_variable   = _name THEN NULL ELSE nights_variable   END,
    sessions_variable = CASE WHEN sessions_variable = _name THEN NULL ELSE sessions_variable END
  WHERE id = v_pricing_id;

  UPDATE pricing_rules SET
    quantity_variable = CASE WHEN quantity_variable = _name THEN NULL ELSE quantity_variable END
  WHERE pricing_id = v_pricing_id;
END;
$function$;

REVOKE ALL ON FUNCTION public.clear_pricing_variable(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.clear_pricing_variable(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.clear_pricing_variable(uuid, text) TO service_role;
...
BEGIN
  ...
  FOR f IN SELECT * FROM fields WHERE product_id = _source ORDER BY display_order LOOP
    IF step_map ? f.step_id::text THEN
      INSERT INTO fields (product_id, step_id, internal_name, variable_name, customer_label, help_text,
        ...
        option_source, catalogue_type, catalogue_id)
  ...
  -- Pricing: copy the pricing record and its component charges only, remapping
  -- each charge to the duplicated component. Quantity mappings are copied only
  -- when the duplicate really has that numeric question, so a stale mapping in
  -- the source never travels into the copy and blocks its activation.
  ...
  IF FOUND THEN
    ...
    UPDATE product_pricing pp SET
      people_variable = CASE WHEN EXISTS (
        SELECT 1 FROM fields f WHERE f.product_id = v_new_id AND f.variable_name = pp.people_variable
          AND f.field_type IN ('quantity','number')) THEN pp.people_variable END,
      days_variable = CASE WHEN EXISTS (
        SELECT 1 FROM fields f WHERE f.product_id = v_new_id AND f.variable_name = pp.days_variable
          AND f.field_type IN ('quantity','number')) THEN pp.days_variable END,
      nights_variable = CASE WHEN EXISTS (
        SELECT 1 FROM fields f WHERE f.product_id = v_new_id AND f.variable_name = pp.nights_variable
          AND f.field_type IN ('quantity','number')) THEN pp.nights_variable END,
      sessions_variable = CASE WHEN EXISTS (
        SELECT 1 FROM fields f WHERE f.product_id = v_new_id AND f.variable_name = pp.sessions_variable
          AND f.field_type IN ('quantity','number')) THEN pp.sessions_variable END
