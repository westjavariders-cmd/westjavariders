CREATE TYPE public.catalogue_source_type AS ENUM ('accommodation_room', 'transport', 'motorbike');

ALTER TABLE public.fields
  ADD COLUMN option_source text NOT NULL DEFAULT 'manual',
  ADD COLUMN catalogue_type public.catalogue_source_type;

ALTER TABLE public.fields
  ADD CONSTRAINT fields_option_source_check CHECK (option_source IN ('manual', 'catalogue')),
  ADD CONSTRAINT fields_catalogue_type_check CHECK (
    (option_source = 'catalogue' AND catalogue_type IS NOT NULL)
    OR (option_source = 'manual' AND catalogue_type IS NULL)
  );

ALTER TABLE public.packages
  ADD COLUMN catalogue_selections jsonb NOT NULL DEFAULT '[]'::jsonb;
  ...
  FOR f IN SELECT * FROM fields WHERE product_id = _source ORDER BY display_order LOOP
    IF step_map ? f.step_id::text THEN
      ...
      VALUES (v_new_id, (step_map ->> f.step_id::text)::uuid, f.internal_name, f.variable_name,
        ...
        f.min_value, f.max_value, f.display_order, f.option_source, f.catalogue_type)
