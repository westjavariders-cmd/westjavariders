-- Product intro (bridge) before the configurator: one photo + editable copy.
...
ALTER TABLE public.product_translations
  ...
  ADD COLUMN IF NOT EXISTS how_it_works_body text,
  ADD COLUMN IF NOT EXISTS what_includes_title text,
  ADD COLUMN IF NOT EXISTS what_includes_body text,
  ADD COLUMN IF NOT EXISTS start_cta_label text;

INSERT INTO storage.buckets (id, name, public)
VALUES ('product-media', 'product-media', false)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "Staff read product media" ON storage.objects
...
BEGIN
  ...
  INSERT INTO products (kind, internal_name, internal_ref, status, sort_order, image_path)
  VALUES (v_src.kind, left('Copy of ' || v_src.internal_name, 200), NULL, 'draft', v_src.sort_order, v_src.image_path)
  RETURNING id INTO v_new_id;

  INSERT INTO product_translations (
    product_id, language_code, title, summary, body, seo_title, seo_description,
    how_it_works_title, how_it_works_body, what_includes_title, what_includes_body, start_cta_label
  )
