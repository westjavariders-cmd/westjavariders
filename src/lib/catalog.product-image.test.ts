...
import { PRODUCT_MEDIA_BUCKET, productConfigureCtaImagePath, productImagePath, productLandingImagePath, productStepImagePath } from "@/lib/catalog";
...
describe("product package image storage", () => {
  ...
  it("keeps the configure-button photo on a distinct path", () => {
    const id = "11111111-1111-1111-1111-111111111111";
    const path = productConfigureCtaImagePath(id, "go.jpg");
    expect(path.startsWith(`${id}/configure-cta/`)).toBe(true);
    expect(path).not.toBe(productLandingImagePath(id, "go.jpg"));
  });
});
